...
void generar_libros(void* nada)
{
	t_config* confImprenta;
	int cadaTiempo,i;

	confImprenta= config_create("/home/utnso/workspace/Ej2/imprenta.conf");

	if(config_has_property(confImprenta, "imprenta_tiempo_generacion"))
	{
		cadaTiempo=config_get_int_value(confImprenta,"imprenta_tiempo_generacion");
	...
	{	printf("Error al leer archivo de configuración de Imprenta!!!\n");}

	config_destroy(confImprenta);
...
int main(void) {
	...
	pthread_t thr_imprenta;


	if(pthread_create(&thr_imprenta, NULL,(void*) generar_libros,NULL)!=0)
	{	printf("Error al Intentar crear hilo Imprenta!!!\n");
		return -1;
	}
	...
	if(pthread_join(thr_imprenta,&ret_imprenta)==0)
	{	return 0;}
